...
var weights_left =   [2,4,3,3,2,2,1,3,2,4,3,3,6]
...
func _ready() -> void:
	...
	for c in $bomb_instance/Games.get_children():
		c.visible = false
	while weight > 0 && current_puzzles.size() < 20: 
		var cont = false
		...
		if !(p == 6 && weight != 1):
			...
			if !matchh:
				...
				if 	puzzle_weights[p] <= weight:
					...
					weights_left.remove_at(p)
					weight -= puzzle_weights[p]
