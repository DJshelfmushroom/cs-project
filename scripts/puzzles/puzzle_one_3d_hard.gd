...
@onready var TextEdit3 = $EquationLabel/AnswerLabel/Num3
var practice = true
var current_text = 1
...
func _ready() -> void:
	completed = false
	$Screen3D.position = Vector3(-0.45,-0.18,0.001)
	$Screen3D.set_size(5,2.5)
	

func _process(_delta: float) -> void:
	if (TextEdit1.text == str($EquationLabel.get_x()) and TextEdit2.text == str($EquationLabel.get_y()) and TextEdit3.text == str($EquationLabel.get_z())):
		TextEdit1.editable = false
		TextEdit1.modulate = Color.GREEN
		TextEdit2.editable = false
		TextEdit2.modulate = Color.GREEN
		TextEdit3.editable = false
		TextEdit3.modulate = Color.GREEN
		$EquationLabel.modulate = Color.LIGHT_GREEN
		$EquationLabel/AnswerLabel.modulate = Color.LIGHT_GREEN
		completed = true
	elif (not(TextEdit1.text == "") && not(TextEdit2.text == "") && not(TextEdit3.text == "") && strikeable == true):
		TextEdit1.editable = false
		TextEdit1.modulate = Color.RED
		TextEdit2.editable = false
		TextEdit2.modulate = Color.RED
		TextEdit3.editable = false
		TextEdit3.modulate = Color.RED
		$"../../..".strikes += 1
		strikeable = false
		...
		TextEdit3.editable = true
		TextEdit3.modulate = Color.WHITE
		strikeable = true
		current_text = 1
	if $"../../..".failed:
		$EquationLabel/AnswerLabel/Num1.editable = false
		$EquationLabel/AnswerLabel/Num2.editable = false
		$EquationLabel/AnswerLabel/Num3.editable = false
...
func _on_button_pressed(num : int):
	...
	elif current_text == 3 and TextEdit3.editable:
		current_text = 0
		TextEdit3.set_text(str(num))
